...
/**
 * Every country code that belongs to a region.
 *
 * The inverse of `regionsOfCountry`, and the backend needs it: regions are
 * derived rather than stored, so a `?regions=cis` filter has to become a list
 * of country codes before it can reach SQL.
 */
export function countriesOfRegion(region: Region): string[] {
  const codes = new Set<string>([...Object.keys(GEOGRAPHIC), ...CIS, ...MUSLIM_WORLD]);

  return [...codes].filter((code) => regionsOfCountry(code).includes(region)).sort();
}
